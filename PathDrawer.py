...
def set_background_with_texture(canvas, texture):
    """
    Fills the given canvas with a tiling texture.
    
    Args:
        canvas (np.ndarray): The canvas where the background will be applied.
        texture (np.ndarray): The tilable texture image.
    
    Returns:
        np.ndarray: The canvas filled with the tiling texture.
    """
    # Get dimensions
